# Ibraheem - Lab 2 Completion

I completed **Lab 2: `02-conflict-check`**.

For this lab, I finished the conflict-checking system using hash-based data
structures so placement checks stay **O(1) expected time**. I implemented the
missing constraints, added support for backtracking with `unplace(...)`, fixed
the input parsing so inline `#` comments are handled correctly, and verified the
program on both the small and large datasets.
